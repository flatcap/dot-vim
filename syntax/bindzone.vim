...
" Extended:     Miek Gieben <miek@miek.nl> (new types, base64)
...
syn keyword     zoneRRType      contained A AAAA CNAME DNAME HINFO MX NS PTR SOA SRV TXT SPF SSHFP TLSA nextgroup=zoneRData skipwhite
syn keyword     zoneRRType      contained DS NSEC3PARAM NAPTR LOC DNAME IPSECKEY HIP nextgroup=zoneRData skipwhite
syn keyword     zoneRRType      contained NSEC NSEC3 RRSIG DNSKEY nextgroup=zoneRDataBase64 skipwhite
syn keyword     zoneRDataRRType contained A AAAA CNAME DNAME HINFO MX NS PTR SOA SRV TXT SPF DS NSEC NSEC3 NSEC3PARAM RRSIG DNSKEY TLSA SSHFP
syn keyword     zoneRDataRRType contained NAPTR LOC DNAME IPSECKEY HIP

syn match       zoneRData       contained /[^;]*/ contains=zoneDomain,zoneIPAddr,zoneIP6Addr,zoneText,zoneNumber,zoneParen,zoneUnknown
syn match       zoneRDataBase64 contained /[^;]*/ contains=zoneDomain,zoneText,zoneNumber,zoneParen,zoneBase64,zoneRDataRRType,zoneUnknown

syn match       zoneIPAddr      contained /\<[0-9]\{1,3}\(\.[0-9]\{1,3}\)\{,3}\>/

" will not match BIND9's pretty printing
syn match       zoneBase64      contained /\([A-Za-z0-9+/]\{4}\)*\([A-Za-z0-9+/]\{2}==\|[A-Za-z0-9+/]\{3}=\)\+/
